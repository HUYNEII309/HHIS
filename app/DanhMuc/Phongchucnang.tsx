...
interface PhongChucNang {
  id: number;
  tenPhong: string;
  moTa: string;
}

const SAMPLE_DATA: PhongChucNang[] = [
  {
    id: 1,
    tenPhong: "Phòng khám tổng quát",
    moTa: "Khám và chẩn đoán bệnh tổng quát",
  },
  {
    id: 2,
    tenPhong: "Phòng răng hàm mặt",
    moTa: "Điều trị và thực hiện các thủ thuật nha khoa",
  },
  {
    id: 3,
    tenPhong: "Phòng X-quang",
    moTa: "Chụp X-quang và chẩn đoán hình ảnh",
  },
  { id: 4, tenPhong: "Phòng phẫu thuật", moTa: "Thực hiện các ca phẫu thuật" },
  {
    id: 5,
    tenPhong: "Phòng điều dưỡng",
    moTa: "Chăm sóc và theo dõi bệnh nhân",
  },
];
...
const Phongchucnang: React.FC = () => {
  const router = useRouter();
  const [data, setData] = useState<PhongChucNang[]>(SAMPLE_DATA);
  const [search, setSearch] = useState("");
  const [formVisible, setFormVisible] = useState(false);
  const [editItem, setEditItem] = useState<PhongChucNang | null>(null);
  const [form, setForm] = useState({ tenPhong: "", moTa: "" });
  const [formError, setFormError] = useState("");

  const filteredData = data.filter(
    (l) =>
      search === "" ||
      l.tenPhong.toLowerCase().includes(search.toLowerCase()) ||
      l.moTa.toLowerCase().includes(search.toLowerCase()),
  );

  const openAdd = () => {
    setEditItem(null);
    setForm({ tenPhong: "", moTa: "" });
    setFormError("");
    setFormVisible(true);
  };

  const openEdit = (item: PhongChucNang) => {
    setEditItem(item);
    setForm({ tenPhong: item.tenPhong, moTa: item.moTa });
    setFormError("");
    setFormVisible(true);
  };
  ...
  const handleSave = () => {
    if (!form.tenPhong.trim()) {
      setFormError("Vui lòng nhập tên phòng chức năng.");
      return;
    }
    const isDuplicate = data.some(
      (l) =>
        l.tenPhong.toLowerCase() === form.tenPhong.trim().toLowerCase() &&
        l.id !== editItem?.id,
    );
    if (isDuplicate) {
      setFormError("Tên phòng chức năng đã tồn tại.");
      return;
    }
    if (editItem) {
      setData((prev) =>
        prev.map((l) =>
          l.id === editItem.id
            ? { ...l, tenPhong: form.tenPhong.trim(), moTa: form.moTa.trim() }
            : l,
        ),
      );
    } else {
      ...
      setData((prev) => [
        ...prev,
        { id: newId, tenPhong: form.tenPhong.trim(), moTa: form.moTa.trim() },
      ]);
    }
    setFormVisible(false);
  };

  const handleDelete = (item: PhongChucNang) => {
    Alert.alert(
      "Xác nhận xoá",
      `Bạn có chắc muốn xoá phòng chức năng "${item.tenPhong}"?`,
      [
    ...
    <View style={styles.container}>
      ...
      <SafeAreaView edges={["top"]} style={styles.header}>
        <View style={styles.headerContent}>
          ...
          <View style={{ flex: 1 }}>
            <Text style={styles.headerTitle}>Phòng chức năng</Text>
            <Text style={styles.headerSub}>Quản lý phòng chức năng</Text>
          </View>
      ...
      <ScrollView
        ...
        <View style={styles.searchCard}>
          <View style={styles.searchBox}>
            ...
            <TextInput
              ...
              placeholder="Tìm phòng chức năng, mô tả..."
        ...
        {filteredData.length === 0 ? (
          <View style={styles.emptyBox}>
            <Ionicons name="business-outline" size={56} color={B.border} />
            <Text style={styles.emptyTitle}>Chưa có phòng chức năng nào</Text>
            <Text style={styles.emptyText}>
              Nhấn nút + để thêm phòng chức năng mới
            </Text>
            <TouchableOpacity style={styles.emptyAddBtn} onPress={openAdd}>
              <Ionicons name="add-circle" size={16} color={B.primary} />
              <Text style={styles.emptyAddText}>Thêm phòng chức năng</Text>
            </TouchableOpacity>
          </View>
        ) : (
                ...
                <View key={item.id} style={styles.loaiCard}>
                  ...
                  <View style={styles.loaiContent}>
                    <View style={styles.loaiTopRow}>
                      ...
                      <View style={{ flex: 1 }}>
                        <Text style={styles.loaiName}>{item.tenPhong}</Text>
                        {item.moTa ? (
              ...
              <View style={styles.modalHeaderRow}>
                <View style={styles.modalHeaderLeft}>
                  <View style={styles.modalHeaderIcon}>
                    <Ionicons name="business" size={18} color={B.primary} />
                  </View>
                  <Text style={styles.modalTitle}>
                    {editItem ? "Sửa phòng chức năng" : "Thêm phòng chức năng"}
                  </Text>
                </View>
                <TouchableOpacity
              ...
              <ScrollView
                ...
                <View style={styles.formField}>
                  ...
                  <View
                    ...
                    <TextInput
                      ...
                      autoFocus
                    />
                    {form.tenPhong.length > 0 && (
                      <TouchableOpacity
                        onPress={() => setForm({ ...form, tenPhong: "" })}>
                        <Ionicons
                          name="close-circle"
                          size={15}
                          color={B.textSub}
                ...
                <View style={[styles.formField, { marginTop: 16 }]}>
                  ...
                  <TextInput
                    ...
                    placeholder="Nhập mô tả phòng chức năng (tuỳ chọn)..."
                    placeholderTextColor={B.textSub}
                    multiline
                    numberOfLines={3}
                  ...
                  <View style={styles.previewBox}>
                    ...
                    <View style={styles.previewCard}>
                      ...
                      <View
                        ...
                        <Ionicons name="business" size={16} color={B.primary} />
                      </View>
                      <View style={{ flex: 1 }}>
                        <Text style={styles.previewName}>{form.tenPhong}</Text>
              ...
              <View style={styles.modalFooter}>
                ...
                <TouchableOpacity style={styles.saveBtn} onPress={handleSave}>
                  ...
                  <Text style={styles.saveBtnText}>
                    {editItem ? "Lưu thay đổi" : "Thêm phòng"}
                  </Text>
                </TouchableOpacity>
              </View>
            </View>
